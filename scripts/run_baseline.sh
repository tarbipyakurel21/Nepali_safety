#!/bin/bash
# Full baseline pipeline for one language stem.
set -euo pipefail
...
sbatch scripts/baseline_infer.sh
echo "After infer job completes, run: bash scripts/judge_stem.sh $STEM results/baseline baseline"
